...
const EditBanner = () => {
  ...
  const fetchBannerDetails = async (id: string) => {
    ...
    if (response) {
      const temp: any = {
        ...
        imageReader: null,
        placement: response.placement.toString(),
        type: response.type.toString(),
        days: response.days,
        startDate: response.expired ? Date.now() : response.startDate,
        endDate: response.expired ? Date.now() : response.endDate,
        expired: response.expired,
      };
      temp.variants = response.variants.map(
        (variant: any) => variant.reference._id
      );
      temp.categories = response.categories.map((item: any) => item.reference);
      temp.brands = response.brands.map((item: any) => item.reference);
      const productData = await fetchProducts();
      const tempFilterData = productData.filter((product: any) =>
        temp.variants.includes(product.id)
      );
      const categoryData = await fetchCategories();
      const tempSelectedCategory = categoryData.filter((item: any) =>
        temp.categories.includes(item.id)
      );
      const brandData = await fetchBrands();
      const tempSelectedBrands = brandData.filter((item: any) =>
        temp.brands.includes(item.id)
      );
      setSelectedCategories(tempSelectedCategory);
      setSelectedBrands(tempSelectedBrands);
      setBannerData(temp);
      setFilteredProducts(tempFilterData);
    }
  };
  const fetchCategories = async () => {
    ...
    if (response) {
      tempCategories = response.records.map((val: any) => ({
        value: val._id,
        title: val.title.replace(/\s*\)\s*/g, ')'),
        image: val.image,
        name: val.title.replace(/\s*\)\s*/g, ')'),
  ...
  const fetchBrands = async () => {
    ...
    const response = await apiService.callAPI();
    if (response) {
      tempBrands = response.records.map((val: any) => ({
        value: val._id,
        title: val.title.replace(/\s*\)\s*/g, ')'),
        image: val.image,
        name: val.title.replace(/\s*\)\s*/g, ')'),
  ...
  const handleSubmit = async () => {
    const tempValidation = { ...validation };
    if (bannerData.title.length === 0 || bannerData.title === '') {
      tempValidation.title = true;
    }
    ...
    if (!Object.values(tempValidation).includes(true)) {
      ...
      const apiService = new APICallService(
        ...
        bannerJson.updateBanner(tempData),
        { id: id },
        '',
        false,
        '',
      ...
      const response = await apiService.callAPI();
      if (response) {
        success(bannerToast.bannerUpdated);
        navigate('/settings/banners', { replace: true });
      }
    }
    setValidation(tempValidation);
    setLoading(false);
                    ...
                    <Row>
                      <Col md={6}>
                        ...
                        >
                          ...
                          <CustomDatePicker
                            className={clsx(
                              'form-control bg-light min-h-60px fs-15 fw-bold text-dark min-w-lg-200px  border rounded border-1',
              ...
              <Card.Body className="pt-0">
                {bannerData?.image ? (
                  ...
                  <div
                    ...
                    //   )}
                    className="upload-btn-wrapper border-r10px  min-w-xl-260px min-h-xl-150px"
                  >
                    <div className="">
                      <img
                        src={UploadSVG}
