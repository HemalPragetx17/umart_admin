...
const EditRecipe = () => {
  ...
  const fetchInitData = async () => {
    ...
    if (response) {
      ...
      const primaryCategory = await fetchPrimaryCategory(2);
      const tempRecipeTypeList = await fetchRecipeTypesList(
        details?.subCategory?.reference
      );
      const allProductsList = await fetchProductList();
      tempRecipeData.subCategory = details?.subCategory?.reference || '';
      tempRecipeData.title = details?.title || '';
      tempRecipeData.serving = details?.serving + '' || '';
      tempRecipeData.cookTime = details?.cookTime + '' || '';
      tempRecipeData.description = details?.description || '';
      tempRecipeData.foodType = details?.foodType + '' || '';
      tempRecipeData.media = details?.media.map((item: any) => {
        return {
          image: item?.url || '',
          reference: item?._id || '',
          imageReader: null,
      ...
      tempRecipeData.recipeType = details?.recipeType.map((item: any) => {
        return {
          title: item?.option || '',
          label: item?.option || '',
          value: item?.option || '',
      ...
      tempRecipeData.ingredients = details?.ingredients.map((item: any) => {
        const categoryObj = primaryCategory.find(
          (cat: any) => cat._id === item?.category?.reference
        );
        ...
        return {
          ...
          subCategory: item?.subCategory?.reference || '',
          variants: item?.variants.map((val: any) => {
            return {
              ...
              image: val?.reference?.media[0]?.url || '',
              id: val?.reference?._id || '',
            };
          }),
          subCategoryList: categoryObj?.subCategory.map((val: any) => {
            return {
              value: val.title,
              name: val.title,
              label: (
          ...
          products: allProductsList
            .filter(
              (productVal: any) =>
                productVal?.product?.category?.reference ===
                  item?.category?.reference &&
                productVal?.product?.subCategory?.reference ===
                  item?.subCategory?.reference
            )
            .map((val: any) => {
              return {
                value: val._id,
      ...
      setCurrentTab({
        ...
        content: (
          <GeneralInfo
            ...
            handleRecipeData={handleRecipeData}
            validations={validations}
            recipeTypeOptions={tempRecipeTypeList}
          />
        ),
        value: 0,
      });
      ...
      setRecipeSteps(details?.recipeSteps);
  ...
  const handleSubmit = async () => {
    ...
    if (isValidGeneralInfo && !isValidIngredients) {
      ...
      const response = await apiCallService.callAPI();
      if (response) {
        success(recipeToast.recipeUpdated);
    ...
    <div>
      {location?.state ? (
        <>
          <Row className="mb-5">
            <Col sm>
              <h1 className="fs-22 fw-bolder">{'Edit recipe'}</h1>
            </Col>
          </Row>
          {!fetchLoader ? (
            <Row>
