...
const AddDiscountPromotion = () => {
  ...
  const fetchDetails = async (id: string) => {
    ...
    if (response) {
      const temp: IDiscountData = {
        ...discountData,
        discountType: response?.discountType
          ? response.discountType.toString()
          : '',
        ...
        applyDiscountTo: response?.applyDiscountTo
          ? response.applyDiscountTo.toString()
          : '',
      };
      if (
        response?.applyDiscountTo &&
        response.applyDiscountTo.toString() === ApplyToProduct
      ) {
        const uniqueCategories: any = {};
        const tempVariant = response.variants.map(
          (item: any, index: number) => {
            const catObj = item?.reference?.product?.category;
            if (!uniqueCategories[catObj.reference]) {
              uniqueCategories[catObj.reference] = {
                label: (
                  <>
                    <img
                      src={catObj?.image || ''}
        ...
        await fetchProductsByCategory(Object.values(uniqueCategories));
        setSelectedProducts(tempVariant);
        setSelectedCategorySimple(Object.values(uniqueCategories));
      }
      if (
        response?.applyDiscountTo &&
        response.applyDiscountTo.toString() === ApplyToBrand
      ) {
        const tempBrands = response.brands.map((item: any, index: number) => ({
          label: (
            <>
              <img
                src={item?.image || ''}
      ...
      if (
        response?.applyDiscountTo &&
        response.applyDiscountTo.toString() === ApplyToCategory
      ) {
        const tempCategories = response.categories.map(
          (item: any, index: number) => ({
            label: (
              <>
                <img
  ...
  const handleSubmit = async () => {
    ...
    if (isValid) {
      setLoading(true);
      const apiCallService = new APICallService(
        promotionCampaign.editDiscountPromotion,
  ...
  const handleChange = (value: string, name: keyof IDiscountData) => {
    ...
    if (name === 'discountType' && value === FlatDiscount) {
      temp.applyDiscountTo = ApplyToCart;
    }
                ...
                <Card.Body>
                        ...
                        <Col xs={12}>
                          <Row className="align-items-center">
                            ...
                            <Col xs={8}>
                              <CustomSelectWhite
                                border={
                                  validation.discountType ? '#e55451' : ''
                                }
                                isDisabled={true}
                                placeholder="Select discount type"
                                options={discountTypeJson}
                                value={discountTypeJson.find(
                                  (item) =>
                                    item.value === discountData.discountType
                                )}
                          ...
                          <>
                            ...
                            {discountData.applyDiscountTo === ApplyToProduct ? (
                              <Col xs={12}>
                                ...
                                <Row className="align-items-start mt-6">
                                  <Col
                                    lg={4}
                                    className="mb-2"
                                  >
                  ...
                  <Row className="mt-6 justify-content-between">
                    ...
                    >
                      <Form.Control
                        className={clsx(
                          'form-control-custom border bg-white  ms-1',
