...
const AddCouponPromotion = () => {
  ...
  const handleSubmit = async () => {
    ...
    tempValidation = {
      ...
      minPurchaseAmount: Validations.isStringEmptyOrZero(
        temp.minPurchaseAmount
      ),
    ...
    if (isValid) {
      setLoading(true);
      const apiCallService = new APICallService(
        promotionCampaign.addCoupon,
        promotionCampaignApiJson.addCoupon(temp, selectedProducts),
        '',
        '',
        false,
        '',
      ...
      const response = await apiCallService.callAPI();
      if (response) {
        success(promotionCampaignToast.couponPromotionAdded);
  ...
  const handleChange = async (value: string, name: keyof ICouponData) => {
    ...
    //     }
    //   }
    // } else if (name === 'redemptionLimit' || name === 'minPurchaseAmount') {
    //   if (Validations.allowOnlyNumbersWithEmptyString(value)) {
    //     temp[name] = value;
    //     tempValidation[name] = false;
    //   } else {
      ...
      <Row className="mb-5">
        <Col sm>
          <h1 className="fs-22 fw-bolder">
            {PromotionAndCampaignString.addCouponPromotion}
          </h1>
        </Col>
      </Row>
      <Row>
                  ...
                  <Row className="justify-content-between">
                    ...
                    >
                      <Card className=" border border-r10px">
                        ...
                        <Card.Body className="pt- text-lg-center">
                          {couponData?.image ? (
...
export default AddCouponPromotion;
