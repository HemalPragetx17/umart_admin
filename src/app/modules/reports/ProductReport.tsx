...
import { inventory, master, reports } from '../../../api/apiEndPoints';
import {
  Inventory,
  PAGE_LIMIT,
  Product,
...
const ProductReport = () => {
  ...
  const [productList, setProductList] = useState<
    {
      ...
      title: string;
      _id: string;
      averageMonthlySalesValue: number;
      currentInventory: number;
      product: any;
      totalInwardedInventory: number;
      totalValue: number;
      totalValueLast30Days: number;
      totalValueLast15Days: number;
      totalValueLast60Days: number;
      totalExpiredAndDamaged: number;
      active: boolean;
      averageMonthlySoldUnits: number;
      lastInwardedAt: string;
      totalSoldUnits: string;
      totalSoldUnitsLast15Days: string;
      totalSoldUnitsLast30Days: string;
      totalSoldUnitsLast60Days: string;
    }[]
  >([]);
  ...
  const handleDownload = async () => {
    ...
    const tempCategory = selectedCategories.filter((val: any) => val.id !== 0);
    if (tempCategory.length > 0) {
      tempCategory.map((val: any, index: number) => {
    ...
    if (response) {
      ...
      link.href = url;
      link.download = 'all-product-report' + '.xlsx';
      link.click();
              ...
              <table className="table table-row-bordered datatable align-middle gs-7 gy-4 my-0">
                <thead>
                  <tr className="fs-16 fw-600 h-65px align-middle">
                    ...
                    <th className="min-w-150px"> Current Inventory </th>
                    <th className="min-w-200px"> Last Inwarded date </th>
                    <th className="min-w-190px">Monthly Sales (TSh)</th>
                    <th className="min-w-190px">Monthly Sales (QTY)</th>
                    <th className="min-w-150px">Total Sales (TSh)</th>
                    <th className="min-w-150px">Total Sales (QTY)</th>
                    <th className="min-w-150px">Sales Last 15 Days (TSh)</th>
                    <th className="min-w-150px">Sales Last 15 Days (QTY)</th>
                    <th className="min-w-150px">Sales Last 30 Days (TSh)</th>
                    <th className="min-w-150px">Sales Last 30 Days (QTY)</th>
                    <th className="min-w-150px">Sales Last 60 Days (TSh)</th>
                    <th className="min-w-150px">Sales Last 60 Days (QTY)</th>
                    <th className="min-w-150px">
                      Total went into expiry & Damage Inventory (QTY)
                    </th>
                  </tr>
                </thead>
                      ...
                      {productList.length ? (
                        productList.map((item, index: number) => (
                          <tr key={item._id}>
                            ...
                            <td>
                              <span className="ms-3">
                                {item?.lastInwardedAt ?  Method.convertDateToDDMMYYYY(
                                  item.lastInwardedAt
                                ) : 'NA'}
                              </span>
                            </td>
                            <td>
                              <span className="ms-3">
                                {Method.formatCurrency(
                                  item?.averageMonthlySalesValue || 0
                                )}
                              </span>
                            </td>
                            <td>
                              <span className="ms-3">
                                {Method.formatCurrency(
                                  item?.averageMonthlySoldUnits || 0
                                )}
                              </span>
                            </td>
                            <td>
                            ...
                            <td>
                              <span className="ms-3">
                                {Method.formatCurrency(
                                  item?.totalSoldUnitsLast15Days || 0
                                )}
                              </span>
                            </td>
                            <td>
                            ...
                            <td>
                              <span className="ms-3">
                                {Method.formatCurrency(
                                  item?.totalSoldUnitsLast30Days || 0
                                )}
                              </span>
                            </td>
                            <td>
                            ...
                            <td>
                              <span className="ms-3">
                                {Method.formatCurrency(
                                  item?.totalSoldUnitsLast60Days || 0
                                )}
                              </span>
                            </td>
