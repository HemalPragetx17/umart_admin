...
const EditRole = () => {
  ...
  const [rolePermissionsDefault, setRolePermissionsDefault] =
    useState<any>(rolePermissions);
  useEffect(() => {
    (async () => {
  ...
  const fetchRoleInfo = async () => {
    ...
    if (response) {
      const temp = { ...response };
      // temp.permissions = temp.permissions.map((item: any) => {
      //   const tempItem = { ...item };
      //   if (
      //     tempItem.permissions.length === 4 ||
      //     tempItem.permissions.includes(0)
      //   ) {
      //     tempItem.permissions = [1, 2, 3, 4, 0];
      //   }
      //   return tempItem;
      // });
      setRole(temp);
      const tempRoles = rolePermissionsDefault;
      tempRoles.map((moduleData: any) => {
        moduleData.permissions.map((item: any, index: number) => {
          const tempPermission = item;
          const checked = isChecked(temp, moduleData, index, item.value);
          tempPermission.isChecked = checked;
          moduleData.permissions[index] = tempPermission;
          moduleData.isAllChecked = isChecked(temp, moduleData, -1, item.value);
        });
      });
      setRolePermissionsDefault(tempRoles);
    }
    setFetchLoading(false);
  };
  const handleCheckboxChange = (
    ...
    let tempRole = { ...role };
    const rolesItem = tempRole.permissions.findIndex((item: any) => {
      return item.module === module;
    });
    if (rolesItem !== -1) {
      if (checked) {
        const tempPermission = tempRole.permissions;
        if (value === All) {
        ...
        tempRole.permissions = tempPermission;
      } else {
        const tempPermission = tempRole.permissions;
  ...
  const isChecked = (
    ...
    const temp = roles.permissions.find((item: any) => {
  ...
  const handleSubmit = async () => {
    const tempValidation = { ...validation };
    const tempRole = { ...role, permission: role.permissions };
    ...
    if (!tempValidation.name && tempRole.permission.length > 0) {
      ...
      const response = await apiService.callAPI();
      if (response) {
        success(masterToast.editRole);
        navigate('/master/roles-permissions', { replace: true });
      }
      setLoading(false);
    }
    setValidation(tempValidation);
    ...
    <>
      <Row>
        <Col
          ...
          className="mb-6"
        >
          <h1 className="fs-22 fw-bolder">{RolesString.editRole}</h1>
        </Col>
        {fetchLoading ? (
            ...
            >
              ...
              <div className="custom-form border border-r10px bg-light p-6 py-9">
                <Row className="align-items-center">
                  ...
                  <>
                    {rolePermissionsDefault.map((moduleData: any) => (
                      <Row
                        className="mb-5"
                            ...
                            {moduleData.permissions.map(
                              (checkbox: any, index: number) => (
                                ...
                                >
                                  ...
                                  />
                                  <label className="form-check-label fs-16 fw-500 text-dark ms-3">
                                    {checkbox.title}
                                  </label>
                                </div>
                              )
