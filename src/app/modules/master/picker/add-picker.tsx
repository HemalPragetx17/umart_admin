...
import { deliveryUser, inventory, pickerEndPoints } from '../../../../api/apiEndPoints';
...
import { APIkey, Add, DeliveryUser, PickerConst } from '../../../../utils/constants';
...
const AddPicker = () => {
  ...
  useEffect(() => {
    (async () => {
      if (!Method.hasPermission(PickerConst, Add, currentUser)) {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.values(validateTemp).every((val) => val === false)) {
      ...
      let apiService = new APICallService(
        pickerEndPoints.addPicker,
        pickerApiJson.addPicker(temp),
        '',
        '',
        false,
        '',
    ...
    <>
      {Method.hasPermission(PickerConst, Add, currentUser) ? (
        <>
          <Row className="mb-6">
            <Col xs={12}>
              <h1 className="fs-22 fw-bolder">{'Add new picker'}</h1>
            </Col>
          </Row>
          <Row>
            <Col
