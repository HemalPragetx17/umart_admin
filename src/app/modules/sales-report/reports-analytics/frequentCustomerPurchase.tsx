...
const FrequestCustomerPurchase = () => {
  ...
  const fetchOrder = async (startDate: string, endDate: string) => {
    ...
    let apiService = new APICallService(
      reports.frequentCustomerReport,
      params,
      '',
      '',
      '',
  ...
  const handleDownload = async () => {
    ...
    if (response) {
      ...
      downloadLink.download =
        'Frequent_Customer_Purchase_Pattern_Analysis_Report' +
  ...
  return (
    <div className="">
      <Row className="g-4">
        <Col md>
          <div className="d-flex align-items-center mt-4">
            <h1 className="fs-22 fw-bolder">
              Frequent Customer Purchase Pattern Analysis Report
            </h1>
          </div>
        </Col>
        <Col md="auto">
        ...
        {!fetchLoading && customerData.length ? (
          <>
            ...
            <table className="table table-row-bordered datatable align-middle gs-7 gy-4 my-0">
              <thead>
                <tr className="fs-16 fw-600 h-65px align-middle">
                  ...
                  <th className="min-w-200px">Order Frequency</th>
                  <th className="min-w-200px">Average Order Value</th>
                </tr>
              </thead>
              <tbody className="fs-15 fw-600">
                {fetchLoading ? (
                  ...
                  <>
                    {customerData.length ? (
                      <>
                        {customerData?.map((val: any, index: number) => (
                          <>
                            <tr key={index}>
                              <td>
...
export default FrequestCustomerPurchase;
